...
@EXPORT    = qw($TE_Loaded $TE_DEBUG $TE_HeurLevel
		timex timex_pos timex_pos2 TE_TagTIMEX TE_AddAttributes Date2ISO
		SetLexTagName SetPosAttName printNonConsumingTIMEXes);
...
#######################################
## Alan Ritter - Nov 13 2008
## simple interface which includes
## POS tagging via Lingua::EN::Tagger 
#######################################
#
...
sub timex {
    ...
    my $tagged_text = $p->add_tags($snt);
    $tagged_text =~ s/<(\w+)>/<lex pos=\"\U$1\">/g;
    $tagged_text =~ s/<\/\w+>/<\/lex>/g;    
	...
	$timex = &TE_AddAttributes(&TE_TagTIMEX($tagged_text), $refDate);
    };
    return $timex;
}

#########################################
## Alan Ritter - Sept 28 2010
## Assumes pos tagging has already been
## done
#########################################
sub timex_pos {
    my ($tagged_text, $refDate) = @_;
    my $timex = "";
    eval {
        $SIG{ALRM} = sub {die "timeout"};
	alarm 2;
...
#########################################
## Alan Ritter - Dec 30 2010
## Different format for POS tags:
## input includes tokenized words and
## pos tags in two different strings
#########################################
sub timex_pos2 {
    my ($text, $pos, $refDate) = @_;
    my @words = split / /, $text;
    my @pos   = split / /, $pos;

    my @tagged;
    for(my $i=0; $i<@words; $i++) {
	push @tagged, "<lex pos=\"$pos[$i]\">$words[$i]</lex>";
    }
	...
	$SIG{ALRM} = sub {die "timeout"};
	alarm 2;
	$timex = &TE_AddAttributes(&TE_TagTIMEX(join(" ", @tagged)), $refDate);
        alarm 0;
    };
    return $timex;    
}
...
sub SetLexTagName {
    ...
    $LexTagName = $TagName;
    $OTCD = "<lex[^>]*$PosAttName=\\\"?CD[^>]*>";
    $OTNNP = "<lex[^>]*$PosAttName=\\\"?NNP?[^>]*>";
...
EACHPAT: while ($string =~ /$curPattern/g){
	...
	my $bef = $`;	
	my $aft = $';	#'	#this is just to fix the syntax highlighting in emacs...
	my $foundString = $1;
...
sub TE_AddAttributes {
	...
	if($Type eq "date") {
		...
		if(!$FoundVal && ($TE_HeurLevel > 1)) {
		    ...
		    if($lead   =~
		       /.*<LexTagName[^>]*$PosAttName=\"?(VBP|VBZ|VBD|MD)[^>]*>(\'?\w+)/sio) {
			$POS = $1; $VB  = lc($2);
			$' =~ /$PosAttName=\"(VB[A-Z]?)\"[^>]*>(\w+)/;
			$POS2 = $1; $verb2 = $2;
		    } elsif($string =~
			    /<$LexTagName[^>]*$PosAttName=\"?(VBP|VBZ|VBD|MD)[^>]*>(\'?\w+)/io) {
			$POS = $1; $VB  = lc($2);
			$' =~ /$PosAttName=\"?(VB[A-Z]?)\"[^>]*>(\w+)/;
			$POS2 = $1; $verb2 = $2;
		    } elsif($output =~
